...
namespace gpi {
    int* generate_gaps_sequence(int array_size, int& sequence_size) {
        ...
        for (int i = 0; i < sequence_size - 1; ++i) {
            for (int j = i + 1; j < sequence_size; ++j) {
                if (sequence[i] < sequence[j]) std::swap(sequence[i], sequence[j]);
            }
        }
        int unique_count = 0;
        for (int i = 0; i < sequence_size; ++i) {
            if (i == 0 || sequence[i] != sequence[i - 1]) sequence[unique_count++] = sequence[i];
        }
    ...
    void shell_sort(std::vector<int>& arr) {
        if (!arr.empty()) {
            shell_sort(arr.data(), static_cast<int>(arr.size()));
        }
    }
}  